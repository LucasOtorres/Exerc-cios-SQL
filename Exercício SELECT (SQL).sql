...
*/
-- O VALOR DA HORA DE UM ESTAGIARIO

  SELECT Valor_Hora 
  from Categoria

-- nome e valor das categorias < 35.000 em ordem crescente de valor 
  
  SELECT Valor_Hora, Categoria
  from Categoria

  WHERE Valor_Hora <= 35.000
  ORDER BY Valor_Hora
...
-- Placa, marca, modelo e ano de fabricação dos carros cadastrados que foram fabricados entre 2000 e 2010 em ordem crescente de marca e modelo
  ...
  WHERE Ano_Fabricacao >= 2000 and Ano_Fabricacao <= 2010

--Placa, marca, modelo e cor dos carros cadastrados adquiridos depois de 2014 em ordem descrescente de ano

   SELECT Placa, Marca, Modelo, Cor,Data_Aquisicao from Veiculo
